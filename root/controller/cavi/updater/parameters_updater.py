...
def update_parameters(data, hyperparameters: HyperparametersModel, variational_parameters: VariationalParameters, starting_parameters : VariationalParameters):
    ...
    # print(variational_parameters.phi_m_k[0:10,:])
...
def update_beta(hyperparameters : HyperparametersModel, sum_phi_k):
    ...
    return sum_phi_k[J:J+T-1] + 1, hyperparameters.gamma + remaining_probs[0:T-1]
...
def update_NIW(y, starting_parameters : VariationalParameters,
    ...
    variational_parameters.nIW.phi = update_NIW_PHI(y, starting_parameters, hyperparameters, sum_phi_k, y_bar, phi_mk)
...
def update_phi_mk(y, variational_parameters : VariationalParameters, T, J):
    ...
    for k in range(0,T-1):
        ...
        e_norm = -1 / 2 * (-jnp.sum(digamma((nu[k] - l) / 2))
                           + jnp.log(jdet(PHI[k, :, :]))
                           + p / lambdA[k] + nu[k]
                           * jnp.diag( ((y - mu[k,:(J+T)]) @ jinv(PHI[k, :, :]) @ (
                                       y - mu[k,:(J+T)]).T)))
    ...
    e_norm_T = -1 / 2 * (-jnp.sum(digamma((nu[T-1] - l) / 2))
                       + jnp.log(jdet(PHI[(T-1), :, :]))
                       + p / lambdA[T-1] + nu[T-1]
                       * jnp.diag( ((y - mu[T-1,:]) @ jinv(PHI[T-1, :, :]) @ (
                    y - mu[T-1,:]).T) ))
    ...
    # print('2',variational_parameters.phi_m_k)
    # print('Norm = ', jnp.sum(variational_parameters.phi_m_k, axis=1))
