...
#
#     nu_0_MIX = np.multiply(np.ones(J), 1000)
#
#     lambda_0_MIX = np.ones(J)*25
#
#
...
#     list = []
#     for i in range(T):
#         list.append(random.choice(Y))
#     mu_var_DP = numpy.array(list)
#     print(mu_var_DP)
...
def specify_user_input(robust_mean, robust_inv_cov_mat, Y):
    ...
    nu_0_MIX = np.multiply(np.ones(J), 1)
    ...
    mu_var_DP = np.tile(mu_0_DP, (T,1))*10
    # print(np.tile(mu_0_DP, (T,1)))
    #mu_var_DP = test_mu_var_DP_init()
    # mu_var_DP = test_mu_var_DP_init_kmeans(Y, T)
